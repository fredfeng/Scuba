...
public class ScubaQuadVisitor extends QuadVisitor.EmptyVisitor {
	...
	public ScubaQuadVisitor(SummaryController controller, AbsHeapHandler ahh) {
	...
	public void visitALoad(Quad stmt) {
		...
		if (ALoad.getDest(stmt) instanceof RegisterOperand) {
			...
			Pair<Boolean, Boolean> flag = new Pair<Boolean, Boolean>(false,
					false);
			flag = ahHandler.handleALoadStmt(meth.getDeclaringClass(), meth,
					lhs.getRegister(), lhs.getType(), lvt, rhs.getRegister(),
					rhs.getType(), rvt);
	...
	public void visitAStore(Quad stmt) {
		...
		if (AStore.getValue(stmt) instanceof RegisterOperand) {
			...
			flag = ahHandler.handleAStoreStmt(meth.getDeclaringClass(), meth,
					lhs.getRegister(), lhs.getType(), lvt, rhs.getRegister(),
					rhs.getType(), rvt);
			sum.getAbsHeap().markChanged(flag);
		}
	...
	public void visitCheckCast(Quad stmt) {
		...
		if (rx instanceof RegisterOperand) {
			...
			if (ro.getType().isReferenceType()) {
				...
				Pair<Boolean, Boolean> flag = new Pair<Boolean, Boolean>(false,
						false);
				flag = ahHandler.handleCheckCastStmt(meth.getDeclaringClass(),
	...
	public void visitGetfield(Quad stmt) {
		...
		if (field.getField().getType() instanceof jq_Reference) {
			...
			Pair<Boolean, Boolean> flag = new Pair<Boolean, Boolean>(false,
					false);
			flag = ahHandler.handleLoadStmt(meth.getDeclaringClass(), meth,
	...
	public void visitGetstatic(Quad stmt) {
		...
		if (field.getField().getType() instanceof jq_Reference) {
			...
			Pair<Boolean, Boolean> flag = new Pair<Boolean, Boolean>(false,
					false);
			flag = ahHandler.handleStatLoadStmt(meth.getDeclaringClass(), meth,
					lhs.getRegister(), lhs.getType(), lvt, field.getField());
			sum.getAbsHeap().markChanged(flag);
		}
	}
	...
	public void visitInvoke(Quad stmt) {
		...
		if (G.tuning)
			StringUtil.reportInfo("handle callsite: " + stmt + " In "
					+ stmt.getMethod() + " Size: " + calleeSumCstPairs.size());
		...
		for (Pair<Summary, BoolExpr> calleeSumCst : calleeSumCstPairs) {
			...
			MemLocInstnItem item = memLocInstnResult
					.get(new Pair<Quad, jq_Method>(stmt, calleeSum.getMethod()));

			// if has not been cached
			if (item == null) {
				item = new MemLocInstnItem(meth, stmt, calleeSum.getMethod(),
						memLocInstnResult);
				memLocInstnResult.put(
						new Pair<Quad, jq_Method>(stmt, calleeSum.getMethod()),
						item);
				...
				for (int i = 0; i < actuals.length(); i++) {
					...
					Register r = ro.getRegister();
					if (sumController.getVarType(meth, r) == VariableType.PARAMETER) {
						actualsMemLoc.add(Env.getParamElem(r, meth,
								meth.getDeclaringClass(), ro.getType()));
					} else if (sumController.getVarType(meth, r) == VariableType.LOCAL_VARIABLE) {
						actualsMemLoc.add(Env.getLocalVarElem(r, meth,
								meth.getDeclaringClass(), ro.getType()));
					} else {
						// actuals can be primitives
						actualsMemLoc.add(Env.getPrimitiveElem());
					}
				}
				// fill the formal-to-actual mapping
				assert (calleeSum.getFormals().size() == actualsMemLoc.size()) : "unmatched actuals and formals list!";
				...
				if (opr instanceof INVOKESTATIC_A
					...
					StackObject sObj = sumController.getMemLocation(
							meth.getDeclaringClass(), meth, ro.getRegister(),
							ro.getType(), sum);
			...
			Pair<Boolean, Boolean> flag = new Pair<Boolean, Boolean>(false,
					false);
			flag = ahHandler.handleInvokeStmt(stmt, calleeSum.getAbsHeap(),
					item, hasTypeCst);
			sum.getAbsHeap().markChanged(flag);
		}
	...
	public void visitMove(Quad stmt) {
		...
		if ((stmt.getOperator() instanceof MOVE_A)
			...
			flag = ahHandler.handleAssignStmt(meth.getDeclaringClass(), meth,
					lhs.getRegister(), lhs.getType(), lvt, rhs.getRegister(),
					rhs.getType(), rvt);
			sum.getAbsHeap().markChanged(flag);
		}
	...
	public void visitNew(Quad stmt) {
		...
		flag = ahHandler.handleNewStmt(meth.getDeclaringClass(), meth,
	...
	public void visitMultiNewArray(Quad stmt) {
		...
		Pair<Boolean, Boolean> flag = new Pair<Boolean, Boolean>(false, false);
		flag = ahHandler.handleMultiNewArrayStmt(meth.getDeclaringClass(),
				meth, rop.getRegister(), rop.getType(), vt, plo.length(), stmt);
		sum.getAbsHeap().markChanged(flag);
	}
	...
	public void visitNewArray(Quad stmt) {
		...
		Pair<Boolean, Boolean> flag = new Pair<Boolean, Boolean>(false, false);
		flag = ahHandler.handleNewArrayStmt(meth.getDeclaringClass(), meth,
				rop.getRegister(), rop.getType(), vt, stmt);
		sum.getAbsHeap().markChanged(flag);
	}
	...
	public void visitPhi(Quad stmt) {
		...
		if (Phi.getDest(stmt) instanceof RegisterOperand) {
			...
			VariableType lvt = sumController
					.getVarType(meth, lhs.getRegister());
			...
			for (RegisterOperand rhs : stmt.getOperator()
				...
				VariableType rvt = sumController.getVarType(meth,
						rhs.getRegister());
				Pair<Boolean, Boolean> flag = new Pair<Boolean, Boolean>(false,
						false);
				flag = ahHandler.handleAssignStmt(meth.getDeclaringClass(),
	...
	public void visitPutfield(Quad stmt) {
		...
		if (f.getField().getType() instanceof jq_Reference) {
			...
			if (rhso instanceof RegisterOperand) {
				...
				Pair<Boolean, Boolean> flag = new Pair<Boolean, Boolean>(false,
						false);
				flag = ahHandler.handleStoreStmt(meth.getDeclaringClass(),
						meth, lhs.getRegister(), lhs.getType(), lvt,
						f.getField(), rhs.getRegister(), rhs.getType(), rvt);
				sum.getAbsHeap().markChanged(flag);
			}
		}
	}
	...
	public void visitPutstatic(Quad stmt) {
		...
		if (field.getField().getType() instanceof jq_Reference) {
			...
			if (rhso instanceof RegisterOperand) {
				...
				Pair<Boolean, Boolean> flag = new Pair<Boolean, Boolean>(false,
						false);
				flag = ahHandler.handleStaticStoreStmt(
						meth.getDeclaringClass(), meth, field.getField(),
						rhs.getRegister(), rhs.getType(), rvt);
				sum.getAbsHeap().markChanged(flag);
			}
		}
	...
	public void visitReturn(Quad stmt) {
		...
		jq_Method meth = stmt.getMethod();
		VariableType type = sumController.getVarType(meth, v);

		Pair<Boolean, Boolean> flag = new Pair<Boolean, Boolean>(false, false);
		flag = ahHandler.handleRetStmt(meth.getDeclaringClass(), meth, v,
				ret.getType(), type);
		sum.getAbsHeap().markChanged(flag);

		assert (sum.getAbsHeap().contains(Env.getRetElem(meth))) : ""
				+ "the return value should be contained in the heap!";
		RetElem retValue = Env.getRetElem(meth);
