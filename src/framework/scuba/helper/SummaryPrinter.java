...
public class SummaryPrinter {
	...
	public static void dumpSummary4Method(jq_Method meth) {
	...
	public static void dumpHeapToFile(AbsHeap absHeap, String count) {
		AbsHeapPrinter.dumpHeapToFile(absHeap, count);
	}

	public static void dumpSumToFile(AbsHeap absHeap, String count) {
		AbsHeapPrinter.dumpSumToFile(absHeap, count);
	}

	public static void dumpPropToFile(AbsHeap absHeap, String count) {
		AbsHeapPrinter.dumpPropToFile(absHeap, count);
	}

	public static void printCalleeHeapInfo(AbsHeap absHeap, String s) {
		int param2Alloc = 0;
		int param2AP = 0;
			...
			for (HeapObject tgt : tgts.keySet()) {
				if (src instanceof ParamElem && tgt instanceof AllocElem) {
					param2Alloc++;
						...
						&& tgt instanceof AccessPathElem) {
					param2AP++;
				} else if (src instanceof StaticFieldElem
					...
					ret2Alloc++;
				} else if (src instanceof RetElem
						&& tgt instanceof AccessPathElem) {
					ret2AP++;
				} else if (src instanceof AccessPathElem
						&& tgt instanceof AllocElem) {
					ap2Alloc++;
				} else if (src instanceof AccessPathElem
						&& tgt instanceof AccessPathElem) {
		...
		for (jq_Method m : sums.keySet()) {
			...
			for (Pair<AbsMemLoc, FieldElem> pair : absHeap.keySet()) {
				...
				for (HeapObject hObj : p2set.keySet()) {
					...
					if (CstManager.isTrue(cst)) {
						t++;
						t_all++;
					} else if (CstManager.isFalse(cst)) {
						f++;
						f_all++;
					} else {
						other++;
