 ...
 */
public class NextKeyHashMapBuilder implements ICFGBuilder {
	@Override
...
//		jq_Field key = ((jq_Class)mapType).getDeclaredField("key");
		jq_Field table = ((jq_Class)mapType).getDeclaredField("table");
		assert(table != null);
		assert(key != null);
		assert(val != null);
		...
		FieldOperand foTable = new FieldOperand(table);
		FieldOperand foKey = new FieldOperand(key);
		FieldOperand foVal = new FieldOperand(val);
//		 GETFIELD_A T15, R0, .table
		Quad q1 = Getfield.create(0, bb, Getfield.GETFIELD_A.INSTANCE, to2, ro0, foTable, null);
